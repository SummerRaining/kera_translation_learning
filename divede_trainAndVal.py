...
#    build_resize()
    #分到train和test中
    SOURCE_PATH = 'C:\\Users\\tunan\\Desktop\\inception-V3-translation\\flower_photos'
    resizeTrainAndTest = "C:\\Users\\tunan\\Desktop\\inception-V3-translation\\valAndTrain"
    if not os.path.exists(resizeTrainAndTest):
        os.mkdir(resizeTrainAndTest)
        print("build resizeTrainAndTest direction")
    os.chdir(resizeTrainAndTest)
    TRAIN_PATH = "train"
    VAL_PATH = "val"
    #切换到训练夹目录下后
    ...
    for cur_path in os.listdir(SOURCE_PATH):
        ...
        train = cur[:int(len(cur)*0.8)]
        val = cur[int(len(cur)*0.8):]
